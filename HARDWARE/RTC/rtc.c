...
{	
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = RTC_IRQPRIORITY;	//先占优先级1位,从优先级3位
...
{	
	...
	if(RTC_GetITStatus(RTC_IT_ALR)!= RESET)//闹钟中断
	{
		...
		vTaskNotifyGiveFromISR(BackGround_Handler,&xHigherPriorityTaskWoken);   //发送任务通知 开启后台任务
